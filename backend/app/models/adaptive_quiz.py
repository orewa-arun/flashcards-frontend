...
These models used to mirror MongoDB documents (with `ObjectId`), but
adaptive quiz state and results are now persisted in PostgreSQL
(`quiz_sessions`, `quiz_results`, `user_flashcard_performance`, etc.).

We keep an optional string `id` in the main document model where
relevant for backwards compatibility, but drop the `bson` dependency.
"""
